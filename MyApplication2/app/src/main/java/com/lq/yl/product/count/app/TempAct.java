...
public class TempAct extends Activity implements View.OnClickListener, SpinerAdapter.OnItemSelectListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.temp_act_layout);
    ...
    public void onItemClick(int pos) {
        ...
        if (mTimeSpace.equals(mListType.get(0))) {
            ArrayList<OrderMdl> list = new OrderDao(TempAct.this).getIntervalList(DateUtils.getCrtDate(), DateUtils.getStateTime(7));
            mBarData = getBarData(list, 12, 100);
        } else if (mTimeSpace.equals(mListType.get(1))) {
            ArrayList<OrderMdl> list = new OrderDao(TempAct.this).getIntervalList(DateUtils.getCrtDate(), DateUtils.getStateTime(30));
            mBarData = getBarData(list, 12, 100);
        } else if (mTimeSpace.equals(mListType.get(2))) {
            ArrayList<OrderMdl> list = new OrderDao(TempAct.this).getIntervalList(DateUtils.getCrtDate(), DateUtils.getStateTime(365));
            mBarData = getBarData(list, 12, 100);
        }
    ...
    private void showBarChart(BarChart barChart, BarData barData) {
        ...
        barChart.setDescription(mTimeSpace + "总收入：" + new OrderDao(TempAct.this).getIntervalTotal(DateUtils.getCrtDate(), DateUtils.getStateTime(7)));// 数据描述
